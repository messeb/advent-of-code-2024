...
The lanternfish would like to know the _sum of all boxes' GPS coordinates_after the robot finishes moving. In the larger example, the sum of all boxes' GPS coordinates is `_10092_`. In the smaller example, the sum is `_2028_`.
...
```bash
python warehouse_woes.py input.txt
```
...
```bash
python warehouse_woes.py input.txt --scaled
```
